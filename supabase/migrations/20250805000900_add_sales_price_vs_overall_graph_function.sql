...
BEGIN
  ...
  WHERE 
    s.sold_at >= (CURRENT_DATE - INTERVAL '1 day' * days_back)
    AND s.price > 0
    AND p.overall IS NOT NULL
    AND p.overall BETWEEN 40 AND 99  -- Filter out unrealistic ratings
  GROUP BY p.overall
  HAVING COUNT(s.id) >= 3  -- Only include ratings with at least 3 sales for statistical relevance
  ORDER BY p.overall;
END;
$function$
;
