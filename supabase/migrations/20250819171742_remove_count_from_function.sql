drop function if exists "public"."test_market_value_updates"(test_count integer);

drop function if exists "public"."update_players_market_values_batch"(batch_size integer, offset_val integer);
...
CREATE OR REPLACE FUNCTION public.update_players_market_values_batch(batch_size integer DEFAULT 5000, offset_val integer DEFAULT 0)
 RETURNS TABLE(processed_count integer, updated_count integer, error_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
