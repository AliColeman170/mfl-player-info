...
CREATE OR REPLACE FUNCTION public.get_favorite_players(limit_count integer DEFAULT 5)
 RETURNS TABLE(id integer, first_name text, last_name text, overall integer, primary_position text, market_value_estimate integer, age integer, club_name text, favorite_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
BEGIN
  ...
  SELECT 
    ...
    p.age,
    p.club_name::TEXT,
    COUNT(f.player_id)::BIGINT as favorite_count
...
CREATE OR REPLACE FUNCTION public.get_top_owners(limit_count integer DEFAULT 5)
 RETURNS TABLE(owner_wallet_address text, owner_name text, player_count bigint, total_value bigint, avg_overall numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(p.market_value_estimate), 0)::BIGINT as total_value,
    ROUND(AVG(p.overall), 0) as avg_overall
  FROM public.players p
  WHERE 
