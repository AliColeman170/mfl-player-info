...
export async function importHistoricalSales(
  ...
  console.log('[Historical Sales] Starting one-time historical sales import...');
  ...
  try {
    ...
    let beforeListingId: number | undefined = lastSaleIdStr ? parseInt(lastSaleIdStr) : undefined;
    
    console.log(`[Historical Sales] ${beforeListingId ? `Resuming from listing ID ${beforeListingId}` : 'Starting fresh historical import'}`);

    let hasMore = true;
    let currentPage = 1;
    const maxPages = 2000; // Safety limit for historical data

    while (hasMore && currentPage <= maxPages) {
      try {
        console.log(`[Historical Sales] Fetching page ${currentPage}${beforeListingId ? `, beforeListingId: ${beforeListingId}` : ''}`);
        ...
        // Process sales batch
        const processingResult = await processSalesBatch(salesPage, executionId);
        ...
        if (lastListingId) {
          await setSyncConfig('last_historical_sale_id', lastListingId.toString());
        }
        ...
        if (salesPage.length < 50) {
          console.log(`[Historical Sales] Received ${salesPage.length} results (less than 50). Last page reached.`);
          hasMore = false;
        } else {
        ...
        console.log(`[Historical Sales] Page ${currentPage - 1} complete: processed ${processingResult.processed}, total processed: ${totalProcessed}`);

        // Rate limiting delay
        await sleep(3000);

      } catch (error) {
        console.error(`[Historical Sales] Error processing page ${currentPage}:`, error);
    ...
    console.log(`[Historical Sales] Completed: fetched ${totalFetched}, processed ${totalProcessed}, failed ${totalFailed}`);
 ...
 */
async function fetchHistoricalSalesPage(beforeListingId?: number): Promise<Listing[]> {
...
async function processSalesBatch(
  ...
  try {
    ...
    if (error) {
      ...
      console.log(`[Historical Sales] Successfully processed ${processed} sales in batch`);
    }
